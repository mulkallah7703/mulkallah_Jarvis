...
export default function ApexWorld() {
  ...
  const { energy } = useMicEnergy(micOn);
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", userSelect: "none" }}>
      ...
      <div style={{ position: "absolute", left: "50%", top: "50%", width: "min(720px, 78vw)", height: "min(300px, 38vh)", transform: "translate(-50%, -56%)", zIndex: 3, pointerEvents: "none" }}>
        <JarvisHologram
      ...
      <div
        role="button"
        tabIndex={0}
        aria-label="Mulkallah Jarvis core - tap to energize"
        onClick={boost}
